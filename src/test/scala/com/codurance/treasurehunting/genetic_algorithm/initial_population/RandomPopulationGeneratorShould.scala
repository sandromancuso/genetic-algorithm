...
class RandomPopulationGeneratorShould extends UnitSpec {
	...
	val INDIVIDUAL_1 = new Individual(actions = Seq(RandomAction.next()))
	val INDIVIDUAL_2 = new Individual(actions = Seq(RandomAction.next()))
	val INDIVIDUAL_3 = new Individual(actions = Seq(RandomAction.next()))
