...
class Evolution(gaConfig: GAConfig,
	...
	def evolve(population: Population): Population = {
		...
		1 to gaConfig.generations foreach { n =>
			...
			println(s"Population of: ${fitPopulation.size()} - Generation: ${n} - Fittest individual: ${fitPopulation.fittestIndividual().fitness} - Gene: PICK_UP_TREASURES: ${pickUpTreasures}")
		}
		fitPopulation
	}
}
